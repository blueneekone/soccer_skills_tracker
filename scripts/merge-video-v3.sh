#!/usr/bin/env bash
# =============================================================================
# SSTRACKER DIGITAL MONETIZATION PIPELINE: FFMPEG SEQUENTIAL STITCHER (v3)
# =============================================================================
# This script programmatically merges, crops, and encodes the raw Playwright v2 clips
...
echo "🎬 [FFmpeg Stitcher v3] Initiating post-processing pipeline..."
...
for SEG in "${SEGMENTS[@]}"; do
  ...
  if [ ! -f "$FILE" ] || [ ! -s "$FILE" ]; then
    ...
    echo "Please ensure you run 'node record-demo-v2.js' before executing this stitcher script."
    exit 1
  fi
done

# 2. Intermediate Encoding & Aspect Ratio Optimization
echo "🎬 [FFmpeg Stitcher v3] Transcoding independent segments to uniform intermediate MP4 streams..."
...
echo "🎬 [FFmpeg Stitcher v3] Constructing concatenation list manifest..."
rm -f "$MANIFEST"
for SEG in "${SEGMENTS[@]}"; do
...
echo "🎬 [FFmpeg Stitcher v3] Concatenating intermediate streams and executing digital crop..."
FINAL_TEMP="$RAW_DIR/merged_unprocessed.mp4"

ffmpeg -y -f concat -safe 0 -i "$MANIFEST" \
  -c copy "$FINAL_TEMP"

echo "🎬 [FFmpeg Stitcher v3] Applying high-fidelity H.264 encoding and mobile-responsive pixel formats..."
...
echo "🧹 [FFmpeg Stitcher v3] Purging intermediate transcode files..."
for SEG in "${SEGMENTS[@]}"; do
...
echo "📊 ==================================================="
echo "📊 PIPELINE ANALYSIS COMPLETE (v3.0):"
