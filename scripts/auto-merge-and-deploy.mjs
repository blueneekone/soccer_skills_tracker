/**
 * SSTracker Automated Continuous Deployment & PR Merge Daemon
 * 
...
async function validateAndMergePr(pr) {
  ...
  try {
    ...
    run(`git pull origin ${pr.headRefName}`);

    // 3. Pre-merge Validation Gate 1: Svelte 5 and TypeScript check
    console.log('\n[Gate 1/4] Running Svelte 5 & TypeScript static analysis (pnpm check)...');
    run('pnpm run check');
    ...
    // 6. Merge PR into dev
    console.log(`\n[Gate 4/4] Validation passed 100% green! Merging PR #${pr.number} into dev...`);
    run('git checkout dev');
    run('git pull origin dev');
    run(`git merge --no-ff origin/${pr.headRefName} -m "chore(merge): auto-merge PR #${pr.number} - ${pr.title}"`);
    ...
    try {
      ...
      console.log(`PR #${pr.number} merged locally into dev.`);
    }
