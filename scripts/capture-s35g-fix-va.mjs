/**
 * One-off VA capture for Sprint 3.5g-a-fix — saves to docs/vision/va-screenshots/
 * Uses persistent e2e profile when available; falls back to env OTP login.
 */
...
async function main() {
	...
	const holoVisible = await holoCard
	...
	// HQ holo — identity stage + adjacent meta column
	...
	const hqPath = await clipScreenshot(
		page,
		identityStage,
		's35g-fix-hq-holo-1280.png',
		1280,
		16,
	);
	console.log('Wrote', hqPath);

	// Armory Studio dossier panel
	...
	const studioPath = await clipScreenshot(
		page,
		dossierPanel,
		's35g-fix-studio-dossier-1280.png',
		1280,
		16,
	);
