...
exports.secureBulkAddPlayers = onCall({region: REGION}, async (request) => {
  ...
  if (!Array.isArray(rawPlayers)) {
    throw new HttpsError('invalid-argument', 'players must be an array.');
  }
  if (rawPlayers.length === 0) {
    throw new HttpsError(
        'invalid-argument',
        'players must contain at least one row.',
    );
  }
  if (rawPlayers.length > MAX_BULK_ROSTER_ROWS) {
    throw new HttpsError(
        'invalid-argument',
        `At most ${MAX_BULK_ROSTER_ROWS} players per bulk import.`,
    );
  }
  ...
  let duplicates = 0;
  let skipped = 0;
  let seatCapHit = false;
  /** @type {Array<{index: number, reason: string}>} */
  const errors = [];
  ...
  for (let index = 0; index < rawPlayers.length; index++) {
    ...
    if (txnResult.kind === 'email_in_use') {
      errors.push({index, reason: 'email_in_use'});
      skipped++;
      continue;
    }
    if (txnResult.kind === 'no_entitlement') {
    ...
    if (txnResult.kind === 'team_full') {
      seatCapHit = true;
      errors.push({index, reason: 'team_full'});
      skipped += rawPlayers.length - index;
      break;
    }
    if (txnResult.kind === 'full') {
      seatCapHit = true;
      errors.push({index, reason: 'club_full'});
      skipped += rawPlayers.length - index;
      break;
    }
  }
...
// ── Epic 5 — Security & Infrastructure: updateUserRole ───────────────────────
//
// Callable for Admins/Directors to securely assign a role and clubId.
// Prevents client-side mutations of protected RBAC fields (Zero-Trust Enforcement).
// ─────────────────────────────────────────────────────────────────────────────
exports.updateUserRole = onCall(
    ...
    async (request) => {
      ...
      const callerRole = request.auth.token.role;
      if (callerRole !== 'global_admin' && callerRole !== 'super_admin' && callerRole !== 'director') {
      ...
      if (data.clubId && typeof data.clubId === 'string') payload.clubId = data.clubId;
      if (data.teamId && typeof data.teamId === 'string') payload.teamId = data.teamId;
      ...
      try {
        ...
        if (data.role === 'coach') {
            const coachLookupRef = db().collection('coach_lookup').doc(data.targetUid);
            batch.set(coachLookupRef, payload, { merge: true });
        }

        await batch.commit();

        logger.info('[updateUserRole] success', {
          actorUid: request.auth.uid,
