...
console.log("🚀 INITIATING SSTRACKER FINAL LAUNCH-NIGHT INFRASTRUCTURE INJECTION (v3)...");
...
`,
  ...
  // 🛰️ WORKFLOW 8: Public Website Overhaul (The CMO / CDO Landing Page)
  '.agents/workflows/jules-builds/audit-public-os.md': `---
name: audit-public-os
description: Asynchronous Cloud VM workflow to overhaul and design the public-facing marketing website.
---
# Swarm Audit: Public Marketing Website (Training Triangle)

@jules, please execute the visual and content overhaul for the public marketing website per the CMO's directive.

### Rules & Gates
1. **Critical Guardrail:** This is a public, prerender = true static route. You are STRICTLY FORBIDDEN from invoking Firebase Auth, minting JWT tokens, or attempting to log in.
2. **Circuit Breaker:** Authorized max of 3 attempts. If failing, revert, log to \\\`/audit-artifacts/public/\\\`, and stop.

### Execution Sequence
- **The Hero Section:** Locate H1 hero text in \\\`src/routes/(marketing)/+page.svelte\\\` and replace with: "Stop managing teams. Start developing athletes. The Youth Sports OS."
- **Interactive Mockups:** Replace any static gray placeholder boxes with a lazy-loaded interactive video loop showcasing the Svelte 5 Dashboard UI.
- **Single CTA Lock:** Ensure there is exactly ONE Action Gold (#fbbf24) primary Call-To-Action button reading "Deploy Your Club" routing to B2B sign-up.
- **Asymmetric Training Triangle Grid:** Implement a strict 12-column asymmetric Bento Grid using fluid clamp math to layout our core decision-makers:
  - Player Development: Spans 6 columns
  - Coach Tactics: Spans 4 columns
  - Parent Shield: Spans 2 columns
- **Color Taxonomy:** Enforce the 60-30-10 color taxonomy: Void Black (#000000) for canvas backgrounds, Navy Slate for panels, and Data Cyan (#14b8a6) for accents. No legacy Tailwind blues are permitted.
- **QA:** Run unauthenticated Playwright visual regression tests to verify the asymmetric layout on desktop, tablet, and mobile widths. Open a non-conflicting PR.
`,
