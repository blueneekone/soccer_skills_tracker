...
4. **B815 Defensive Hydration**: Every raw Firestore `getDocs()` or `onSnapshot()` database transaction on both SvelteKit pages and services must be wrapped in an early-return check to prevent Firebase Quota Exceeded loops. Ensure `authStore` is explicitly imported from `$lib/stores/auth/facade.svelte`:
   ```typescript
   import { authStore } from '$lib/stores/auth/facade.svelte';

   if (!db || !authStore.isAuthenticated) return;
   ```
5. **Crypto SSR Guarding**: When removing fallback checks for `crypto` (e.g., `typeof crypto !== 'undefined'`), ensure the execution block is guarded against server-side rendering (SSR) environments (e.g., using a `!browser` check) to prevent runtime crashes in older Node versions or non-secure HTTP contexts.
6. **Global Cloud Functions**: Ensure any new globally available Cloud Function is exported in both `functions/index.js` and `functions-platform/index.js` to correctly propagate across the split backend codebases.
7. **Pessimistic Definition of Done**: A persona's logic is only "Done" when the compiler returns exactly 0 Svelte compilation errors, 0 TypeScript `any` violations, and the unit tests are 100% green. **We do not merge red pipelines.**
...
*   **Backend Verification Tasks**:
    ...
    *   Verify that `updateUserRole` is a secure, server-side Cloud Function and that unauthenticated clients have absolutely zero client-side `updateDoc` capabilities to set the `role` field on a user document using the canonical map diffing pattern: `!request.resource.data.diff(resource == null ? {} : resource.data).affectedKeys().hasAny(['role'])`.
...
*   **Backend Verification Tasks**:
    ...
    *   Audit the ODP Talent Pipeline data flow. Confirm that player physical telemetry (1000Hz metrics) is correctly mapped to the 6-axis data array in the strict order `[PACE, ACCEL, AGILITY, STAMINA, POWER, COMP]` before being sent to the client.
    *   Ensure that absolutely NO client-side data mutations exist in this workspace. Ensure that 'Action Gold' (`#fbbf24`) accents are not used in these components.

### 3. Director OS (B2B Revenue Engine)
*   **Target Route**: `src/routes/(app)/director/` & `functions/src/domains/interoperabilityOps.js`
*   **Backend Verification Tasks**:
    *   Verify **The Vampire Importer** CSV parser. It must write raw legacy roster data to `roster_staging` in atomic batches capped at a hard limit of **500 operations** per Firestore transaction. Ensure N+1 queries chunk batches up to 30 items using the `in` operator.
    ...
    *   Ensure all Cloud Functions for the 'director' persona utilize the `assertDirectorOrSuper` or `assertDirectorClubOrSuper` role-verification utilities extracted from `functions/src/middleware/authBouncers.js`.
*   **TDD Test Suite**: Run `pnpm test functions/vampire` and assert cursor-based batch pagination (`startAfter` + `limit(500)`).

### 4. Coach OS (The Sideline SIEM)
*   **Target Route**: `src/routes/(app)/coach/` & `functions/src/domains/commsOps.js`
*   **Backend Verification Tasks**:
    *   **Legally Mandated SafeSport Shadow CC**: Remove any client-side parent email resolution logic. Ensure `memberIds` strictly contains two elements (the sender and receiver). Write or verify the `onChannelCreated` Firestore `onCreate` trigger:
        1. When an adult coach creates a chat channel, initialize it with `channelStatus: 'BLOCKED_VPC_PENDING'` (Zero-State Lock).
        2. Identify any player under the age of 18.
        3. Resolve the linked guardian emails and dynamically write them to the separate `ccParentEmails` array. Ensure read access via `request.auth.token.email in resource.data.ccParentEmails`.
        4. Once resolved, promote `channelStatus` to `'ACTIVE'`. Message dispatch must block execution unless `channelStatus === 'ACTIVE'`.
...
*   **Backend Verification Tasks**:
    ...
    *   **Loss Avoidance (Daily Streak Decay)**: Audit the daily cron job that reduces a player's physical metrics by **2%** after 24 hours of inactivity. Ensure this job queries and mutates the nested `armory` map within the canonical `users` collection (e.g., `armory.totalXP`, `armory.streakFreeze.available`, `armory.stats.scoutsSix`). Isolated legacy collections like `player_stats` are banned.
...
# Scope any global modifications strictly to the relevant files. DO NOT remove `.skip` modifiers from unrelated test suites or globally modify unrelated code via `sed -i`.
# Re-run pnpm test until 100% green

# 4. Trigger PR Tracking for unmodified verified files
# To include an unmodified server-side trigger file in a PR patch for architectural verification during code review, "touch" the file by prepending a high-level compliance or architectural comment block (e.g., `// 🛡️ SafeSport Compliance Mandate...`) so Git registers the file as modified.

# 5. Update the Roadmap upon completion
