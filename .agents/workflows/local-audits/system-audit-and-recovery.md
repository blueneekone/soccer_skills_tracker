...
For each architectural violation or broken component identified during the gap analysis, generate a strict, 80-line-capped TDD workflow file designed to fix it.
CRITICAL DIRECTORY ROUTING: You MUST save all newly generated workflow files directly into the .agents/workflows/jules-builds/ directory. Do NOT save them in the root directory or the local-audits folder.
Ensure each generated workflow file includes the required SKILL.md constraints and instructs Jules to run the test suite before completion.