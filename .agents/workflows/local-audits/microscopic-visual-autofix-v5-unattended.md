---
name: microscopic-visual-autofix-v5-unattended
description: HEADLESS UNATTENDED UI HEALING WORKFLOW. Enforces strict design-token.css compliance, prevents layout erasures/zero-margins, standardizes data tables, and locks exact button aesthetics across Svelte 5 viewports without human validation.
---

# 🛡️ UNATTENDED WORKFLOW: COHESIVE VISUAL HEALING & STYLING LOCK (v5.0)

This workflow legally binds the Antigravity/Jules execution loops to perform a strict, physical, browser-in-the-loop visual audit and structural repair against **exactly ONE persona at a time**, with absolutely zero human-in-the-loop pauses [cite: 1115]. 
...
*   Usage: `/microscopic-visual-autofix-v5-unattended [Persona]` (e.g., `/microscopic-visual-autofix-v5-unattended Admin` or `/microscopic-visual-autofix-v5-unattended Player`) [cite: 1115].
...
1. The agent must verify if `src/design-tokens.css` exists in the repository.
2. If it is missing, the agent **MUST** programmatically copy `/workspace/artifacts/design-tokens.css` directly to `src/design-tokens.css` [cite: 782, 803].
...
*   **The Law:** The gap between Bento cards and layout components **MUST NEVER** drop below `12px` or `clamp(12px, 2vw, 24px)` [cite: 782, 1140].
*   **The Grid:** All dashboards must use the 12-column asymmetric Bento Grid topology locked with fluid clamp math [cite: 782, 1140]:
   ```css
   grid-template-columns: repeat(auto-fit, minmax(min(100%, clamp(280px, 30vw, 350px)), 1fr));
   ```
*   **The Margin:** No component or page shell is permitted to sit directly against a navigation menu [cite: user_query]. All pages must enforce a minimum of `tw-p-6` or `clamp(16px, 3vw, 24px)` outer spacing [cite: 1140].
...
### 4. Cohesive Button Taxonomy
Buttons must be strictly locked to their persona profiles to prevent choice paralysis [cite: 808, 1138]:
...
If any visual overlaps or styling deviations are detected:
...
3. **Assert Spacing Math:** Replace any hardcoded `p-0` or `m-0` layout hacks with fluid clamp spacing (`clamp(12px, 2vw, 24px)`) [cite: 1140].
...
2. Commit the styling adjustments to your branch using our strict automated author identity to avoid self-triggering loop traps [cite: 835]:
   ...
   git add .
   git commit -m \"style: unattended visual styling lock and unified design token compliance for [Persona] OS\"
   ``` [cite: 835, 1115]
