...
Emotional Design & Peak-End Rule: You are strictly forbidden from rendering raw, machine-like error codes or dead-end empty states. You must identify "ownable moments" in high-anxiety workflows (e.g., failed payments, compliance bottlenecks) [cite: 719, 720]. You must intercept these valleys with warm, highly actionable recovery interfaces. The conclusion of a high-stress administrative task MUST fire an asynchronous, hardware-accelerated "High-Five" visual validation (e.g., canvas-confetti) immediately following a successful Firebase atomic commit to shift the user's emotional state to satisfaction [cite: 719, 803].