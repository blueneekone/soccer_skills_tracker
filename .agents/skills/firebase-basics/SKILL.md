...
If yes: Set it as the active project and add a default alias by running:bash
...
Use npx for CLI commands: To ensure you always use the latest version of the Firebase CLI, always prepend commands with npx -y firebase-tools@latest instead of just firebase. (e.g., use npx -y firebase-tools@latest --version instead of firebase --version).   

Prioritize official knowledge: For any Firebase-related knowledge, consult the developerknowledge_search_documents MCP tool before falling back to Google Search or your internal knowledge base. Including "Firebase" in your search query significantly improves relevance. When querying, strictly target the following reference directories: "Firebase core concepts," "Firebase CLI usage," "Firebase client library usage," "Firebase CLI and MCP server," "Firebase IaC usage," and "Firebase security-related features".   

Explicit Command Templates: Format CLI requests exactly as required to prevent syntax hallucination. For example, use explicit templates such as npx -y firebase-tools@latest projects:create <PROJECT_ID> --display-name <DISPLAY_NAME>.   
