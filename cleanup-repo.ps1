...
# Run this once in your PowerShell terminal before pushing changes to GitHub [cite: 620].
# ==============================================================================
...
# 2. Sweep old versions of launch automation scripts (v1 to v40) [cite: 144]
Write-Host "[*] Archiving obsolete launch scripts (v1 through v40)..." -ForegroundColor Gray
$ScriptsToClean = Get-ChildItem -Filter "run-launch-automation-v*.ps1" | Where-Object {
    ...
    $Version -lt 41 # Keep the finalized v41 script active [cite: 144]
}
...
$OldSequentials = Get-ChildItem -Filter "run-sequential-local-v*.ps1" | Where-Object {
    ...
    $Version = [int]$Matches[1]
    $Version -lt 6 # Archive older iterations, preserve v6 if needed
}
...
# 5. Lock Git Configurations to prevent untracked file pollution [cite: 216]
if (Test-Path ".gitignore") {
    $GitIgnoreContent = Get-Content ".gitignore"
    if ($GitIgnoreContent -notcontains "audit-artifacts/") {
        Add-Content -Path ".gitignore" -Value "audit-artifacts/"
        Write-Host "[+] Added audit-artifacts/ to .gitignore [cite: 216]." -ForegroundColor Green
    }
...
Write-Host "✨ Cleanup Complete! Deprecated files moved to .deprecated_backups/." -ForegroundColor Green
Write-Host "👉 Running 'git status' to inspect current clean repository layout." -ForegroundColor Cyan
git status
