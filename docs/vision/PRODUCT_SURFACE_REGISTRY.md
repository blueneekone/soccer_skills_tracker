...
5. **Nav policy** — **Tier 1** = pre-acquisition required for functional sign-off (or explicit waiver in §4). **Tier 2+** routes may remain in sidebar per §2; they are waivable and not exec-cut blockers per §0 rules 2–3. `nav_visible=false` = hidden from primary nav (deep-link only). Do not remove Tier 2 coach/parent links from §2; they are intentional and waivable.
...
| PS-C04   | coach     | `/coach/tactical`         | War Room            | 2    | true        | —         | —                                      | —                          | `coach-bento-12col`     | `[COACH_OS_FOUNDATION.md](./COACH_OS_FOUNDATION.md)`   | —                                                                     | Flat sideline analytics | `coach/tactical/+page.svelte`          | Canonical tactical surface — `TacticalArena` + `createTacticalWarRoom`; sidebar + HQ hero (**WARROOM-SINGLE-SURFACE**) |
| PS-C05   | coach     | `/coach/tactics-board`    | Tactics board       | —    | **removed** | —         | —                                      | —                          | —                       | `[COACH_OS_FOUNDATION.md](./COACH_OS_FOUNDATION.md)`   | —                                                                     | Merged into War Room    | redirect → `/coach/tactical` | **WARROOM-SINGLE-SURFACE** — legacy `TacticalCommandBoard` retired; redirect only |
...
| War Room        | `/coach/tactical`      | PS-C04      |
| Match Day       | `/coach/match-day`     | PS-C06      |
| Scouting        | `/coach/scouting`      | PS-C07      |
| Team Ops        | `/coach/logistics`     | PS-C08      |


**Excluded from nav:** ~~Trial Builder (`/coach/trial-builder`, PS-C09)~~ **REMOVED** — merged into Scouting roster eval tab (**SURFACE-MERGE-TRIAL-EVAL**); redirect only. ~~Tactics board (`/coach/tactics-board`, PS-C05)~~ **REMOVED** — merged into War Room (**WARROOM-SINGLE-SURFACE**); redirect only.
...
| ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| `[COACH_OS.md](./COACH_OS.md)` § Home screen zones                      | Tactical `/coach/tactical` equal home-zone pillar                                           | Tier 2; `nav_visible=true`; HQ hero CTA enabled                              | **Aligned** — War Room visible per **WARROOM-SINGLE-SURFACE**     |
| `[FUNCTIONAL_AUDIT_BACKLOG.md](../FUNCTIONAL_AUDIT_BACKLOG.md)` F2 Done | “War Room in `workspaceNav.js`”                                                             | War Room in coach sidebar                                                    | **Aligned** — **WARROOM-SINGLE-SURFACE**                          |
...
| `[ROADMAP.md](../../ROADMAP.md)` open Coach OS work                     | Tactical/assignments hardening TBD                                                          | War Room single surface shipped                                              | **Resolved** — **WARROOM-SINGLE-SURFACE Done**                    |
