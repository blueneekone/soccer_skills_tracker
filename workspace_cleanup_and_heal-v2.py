        ...
        for file in files:
            ...
            try:
                ...
                if "onIdTokenChanged" in content and "SameSite=Strict" not in content:
                    lines = content.splitlines()
                    for idx, line in enumerate(lines):
                        # ONLY match if it is an actual function listener, NOT an import or export line
                        if "onIdTokenChanged" in line and "import" not in line and "export" not in line and "from" not in line:
                            indentation = " " * (len(line) - len(line.lstrip()))
                            cookie_sync = (
...
def main():
    print_status("SSTracker Master Cleanup & Local Recovery Engine [v2] Online", Colors.HEADER)
    ...
    print("  2. Applies local codebase repairs (color swaps to Amber, Svelte Sign-In repairs, token refreshes, and Svelte 5 navigation guards) with safe import filtering.\n")
