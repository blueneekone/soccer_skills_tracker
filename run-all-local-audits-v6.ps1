# SSTracker Smarter Local Visual Audit & Fix Pipeline (v6) - Unattended Edition
...
# Includes fully headless visual auto-fixes without human-in-the-loop pauses.
...
# Aligned Persona definitions (matches exact uids specified in visual-regression-v5.spec.ts)
$Personas = @(
    [PSCustomObject]@{ Name = "public"; Route = "/"; Role = "public"; Uid = "mock-public-uid" },
    [PSCustomObject]@{ Name = "admin"; Route = "/admin/overview"; Role = "admin"; Uid = "admin-telemetry-uid" },
    [PSCustomObject]@{ Name = "director"; Route = "/director/dashboard"; Role = "director"; Uid = "director-telemetry-uid" },
    [PSCustomObject]@{ Name = "coach"; Route = "/coach/dashboard"; Role = "coach"; Uid = "coach-telemetry-uid" },
    [PSCustomObject]@{ Name = "player"; Route = "/player/dashboard"; Role = "player"; Uid = "player-telemetry-uid" },
    [PSCustomObject]@{ Name = "parent"; Route = "/parent/dashboard"; Role = "parent"; Uid = "parent-telemetry-uid" }
)

Write-Host "=============================================" -ForegroundColor Green
Write-Host "   SSTRACKER UNIFIED LOCAL VISUAL AUDITOR v6" -ForegroundColor Green
Write-Host "=============================================" -ForegroundColor Green
...
if (-not $SkipPortCheck) {
    ...
    if (-not $SvelteActive) {
        ...
        Start-Process -FilePath "powershell.exe" -ArgumentList "-NoProfile", "-Command", "pnpm run dev" -NoNewWindow
        # Poll Svelte server
...
foreach ($Persona in $Personas) {
    ...
    Write-Host "[AUDIT] Initiating browser-in-the-loop visual verification via audit-computed-styles-v6.js..." -ForegroundColor Yellow
    $env:AUDIT_TARGET = $Persona.Name
    
    try {
        # Run Playwright test runner synchronously via cmd.exe shim (to avoid Windows path issues)
        $AuditProcess = Start-Process -FilePath "cmd.exe" -ArgumentList "/c pnpm playwright test audit-computed-styles-v6.js --project=desktop-chrome" -NoNewWindow -PassThru -Wait
        if ($AuditProcess.ExitCode -eq 0) {
            ...
            # 5. Local Antigravity CLI Auto-Fix (Headless & Unattended v4)
            Write-Host "[HEAL] Invoking local Antigravity CLI to auto-heal visual layouts (Headless v4)..." -ForegroundColor Yellow
            $HealProcess = Start-Process -FilePath "powershell.exe" -ArgumentList "-NoProfile", "-Command", "agy -p '/microscopic-visual-autofix-v4-unattended $($Persona.Name)'" -NoNewWindow -PassThru -Wait
            if ($HealProcess.ExitCode -eq 0) {
                Write-Host "[HEAL] Headless auto-heal pass completed. Re-running validation..." -ForegroundColor Yellow
                $RetryProcess = Start-Process -FilePath "cmd.exe" -ArgumentList "/c pnpm playwright test audit-computed-styles-v6.js --project=desktop-chrome" -NoNewWindow -PassThru -Wait
