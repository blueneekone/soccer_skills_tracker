...
function parseFacilityWeatherPayload(body) {
  ...
  let clubId = typeof r.clubId === "string" ? r.clubId.trim() : "";
  let facilityId = typeof r.facilityId === "string" ? r.facilityId.trim() : "";
  const params =
...
exports.facilityWeatherWebhook = onRequest(
  ...
  async (req, res) => {
    ...
    if (!clubId || !facilityId) {
      logger.warn("facilityWeatherWebhook: missing clubId/facilityId", {
        clubId,
        facilityId,
        ruleName,
      });
