...
import {
	...
	downloadIcsFile,
	initSignatureCanvas
} from './modules/tracker.js';
import { initDirectorModule, loadComplianceDashboard } from './modules/director.js';
import {
	initCoachDropdown,
	loadCoachDashboard,
	currentCoachTeamId,
...
window.getAppContext = () => {
	...
	const effectiveTid =
		userProfile.role === 'super_admin' || userProfile.role === 'director'
...
window.navigateTo = (viewId, addToHistory = true) => {
	...
	const tokenRole = userProfile?.claimsRole || role;

	if (viewId === 'viewAdmin' && tokenRole !== 'super_admin') {
		alert('Unauthorized Access: Super Admin required.');
		return window.navigateTo('viewHome', false);
	}
	if (viewId === 'viewDirector' && tokenRole !== 'super_admin' && tokenRole !== 'director') {
		alert('Unauthorized Access: Director required.');
		return window.navigateTo('viewHome', false);
	}
	if (viewId === 'viewCoach' && (tokenRole === 'player' || tokenRole === 'parent')) {
...
function checkRoles(user) {
	...
	const hasCoachAccess =
		myTeams.length > 0 || userProfile.role === 'super_admin' || userProfile.role === 'director';
	if (hasCoachAccess) {
		const coachBtn = document.getElementById('btnHomeCoach');
		if (coachBtn) coachBtn.classList.remove('d-none');
		const teamsToPass =
			userProfile.role === 'super_admin' || userProfile.role === 'director' ? globalTeams : myTeams;
...
async function loadHomeDashboard() {
	...
	if (schedList) {
		if (!tid) schedList.innerHTML = "<li class='session-empty'>Select a team to view schedule.</li>";
			...
			schedList.innerHTML =
				events
					.map(
						(e) => `<li class="session-item">
                <div><b style="color:var(--aggie-blue);">${e.type}</b>: ${e.location}<br>
                <span style="font-size:11px; color:#64748b;">${e.date} @ ${e.time}</span></div></li>`
					)
					.join('') || "<li class='session-empty'>No upcoming events.</li>";
		}
	}
	...
	if (hwList) {
		if (role === 'super_admin' || role === 'director') {
			hwList.innerHTML =
				"<li class='session-empty' style='color:#ea580c;'>Admins don't receive personal homework.</li>";
		} else {
			...
			snap2.forEach((d) => {
				const hw = d.data();
				if (hw.status === 'active') {
					let drillSummary = Array.isArray(hw.drills)
						? hw.drills
								.map(
									(dr) =>
										`• ${dr.name} <span style="color:#64748b;">(${dr.sets}x${dr.reps})</span>`
								)
								.join('<br>')
						: hw.drill;
...
const initApp = () => {
	...
	['navHome', 'navTrack', 'navStats', 'navTrophy', 'navCoach', 'navAdmin'].forEach((nid, i) => {
		safeBind(nid, 'click', () =>
			window.navigateTo(
				['viewHome', 'viewTracker', 'viewStats', 'viewTrophy', 'viewCoach', 'viewAdmin', 'viewChallenge'][i]
			)
		);
	});
	...
	safeBind('addAdminBtn', 'click', () =>
		addAdmin(globalAdmins, () => {
			fetchConfig();
			renderAdminTables(
				window.globalClubs,
				globalTeams,
	...
	safeBind('submitWorkoutBtn', 'click', () =>
		handleWorkoutSubmit(userProfile, globalTeams, () => window.navigateTo('viewHome'))
	);
	...
	document.body.addEventListener('click', async (e) => {
		...
		if (tab && !tab.id.startsWith('btn-coachTab')) {
			...
			if (targetId === 'dir-section-compliance') {
				loadComplianceDashboard(db, userProfile.clubId);
			}
		}
		...
		if (target.classList.contains('action-remove-hw-drill')) {
			...
			if (window.currentHomeworkBuilder.length === 0)
				list.innerHTML = '<li class="session-empty">No drills added.</li>';
			else
				list.innerHTML = window.currentHomeworkBuilder
					.map(
						(item, i) =>
							`<li style="padding:5px; border-bottom:1px solid #eee; font-size:12px; display:flex; justify-content:space-between; align-items:center;"><span><b>${i + 1}.</b> ${item.name}</span><button class="delete-btn action-remove-hw-drill" data-index="${i}">✕</button></li>`
					)
					.join('');
		}
	});
		...
		try {
			...
			if (userRole === 'super_admin' || userRole === 'director') {
				userProfile = {
					...baseProfile,
					clubId:
						(baseProfile && baseProfile.clubId) ||
						(window.globalClubs && window.globalClubs[0]?.id
							? window.globalClubs[0].id
							: 'aggiesfc'),
					teamId: 'admin',
			...
			if (isProfileComplete) {
				...
				await safeLoad('Workouts', async () => {
					if (window.fetchWorkouts) await window.fetchWorkouts();
				});
				await safeLoad('Coach Tools', async () => {
					if (window.buildCoachDropdowns) window.buildCoachDropdowns();
				});
				await safeLoad('Director', async () => {
					if (typeof initDirectorModule !== 'undefined')
				...
				if (typeof initSetupDropdowns !== 'undefined')
