...
export function formatHouseholdGraphLabel(
	...
	if (isParentRole(row.role)) {
		const players = Array.isArray(household?.playerEmails)
			...
			: [];
		if (players.length === 0) return row.householdId ? 'Household (no athletes)' : '—';
...
export async function enrichUsersWithHouseholdGraph(
	...
	const householdIds = [...new Set(rows.map((r) => r.householdId).filter(Boolean))] as string[];
	...
	await Promise.all(
		householdIds.map(async (hid) => {
			try {
				...
				if (snap.exists()) householdById.set(hid, snap.data() as Record<string, unknown>);
			} catch {
				/* non-fatal */
			}
		}),
	...
	const playerEmails = rows.filter((r) => isPlayerRole(r.role)).map((r) => r.email);
	const lookupGuardians = await fetchGuardiansFromPlayerLookup(db, playerEmails);

	return rows.map((row) => {
		const household = row.householdId ? householdById.get(row.householdId) ?? null : null;
