...
describe.skipIf(!process.env.FIRESTORE_EMULATOR_HOST)('LAUNCH-test-integrity — Loop Integrity Guards G1–G5, G7–G10 (emulator)', () => {
  ...
  it('G1: player reads OWN player_stats doc (uid == playerKey) — succeeds', async () => {
    const db = env.authenticatedContext('uid-player-a', token({
  ...
  it('G1: cross-club director read of player_stats is denied (tokenClubMatchesDoc fails)', async () => {
    ...
    const db = env.authenticatedContext('director-b', token({
  ...
  it('G3: parent with non-matching householdId reads child users/{email} doc — denied', async () => {
    ...
    const db = env.authenticatedContext('parent-other', token({
  ...
  it('G5: director reads own club license_entitlement — succeeds', async () => {
    ...
    const db = env.authenticatedContext('director-a', token({
  ...
  it('G7: global_admin reads rl_transitions — succeeds (isSuper())', async () => {
    const db = env.authenticatedContext('super-uid', token({
  ...
  it('G7: client write to rl_transitions — denied even for global_admin (allow write: if false)', async () => {
    ...
    await assertFails(setDoc(doc(db, 'rl_transitions/rt-new'), {
      workoutId: 'w-x'
    }));
  });
  ...
  it('G9: parent WITHOUT JWT or users doc householdId reads child users/{email} doc — denied', async () => {
    ...
    const db = env.authenticatedContext('parent-nohh', token({
  ...
  it('G12b: parent cannot read staff_internal message — denied', async () => {
    ...
    await assertFails(getDoc(doc(db, 'clubs/club-a/channels/staff-internal-team-a/messages/msg-staff-1')));
  });
  it('G12c: coach on team reads staff_internal message — succeeds', async () => {
    const db = env.authenticatedContext('coach-uid', token({
    ...
    await assertSucceeds(getDoc(doc(db, 'clubs/club-a/channels/staff-internal-team-a/messages/msg-staff-1')));
  });
  ...
  it("G11b: parent lists completion_verifications by householdId query — denied", async () => {
    ...
    const q = query(collection(db, 'completion_verifications'), where('householdId', '==', 'hh-a'), where('status', '==', 'pending'));
