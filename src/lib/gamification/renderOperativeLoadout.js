 ...
 * @returns {string}
 */
export function renderLoadoutBannerLayer(itemId, size = 128) {
	if (!itemId) return '';
	const entry = getLoadoutCatalog().find((row) => row.id === itemId && row.slot === 'banner');
	if (!entry) return '';
	if (entry.assetPath) {
		return renderManifestAsset(entry, size, 'loadout-banner');
	}
	return '';
}
...
/**
 * Compose portrait base + optional border/badge/banner overlays for dossier preview.
 *
 * @param {{
...
export function composeOperativePortrait({ operativeAvatar, loadout, size = 128, ownedIds }) {
	const s = Math.max(1, Math.floor(Number(size) || 128));
	const portraitSvg = renderOperativeAvatarSvg(operativeAvatar ?? 'operative', s);
