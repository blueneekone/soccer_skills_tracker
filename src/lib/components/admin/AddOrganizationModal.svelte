...
{#if isOpen}
	...
	>
		...
		<!-- svelte-ignore a11y_no_static_element_interactions -->
		<div 
			class="tw-bg-[#0f172a] tw-border tw-border-[#334155] tw-rounded-2xl tw-p-6 tw-max-w-2xl tw-w-full tw-max-h-[90vh] tw-overflow-y-auto"
			onclick={(e) => e.stopPropagation()}
		>
			<div class="tw-flex tw-justify-between tw-items-center tw-mb-6">
				<h2 class="tw-text-[#FAFAFA] tw-text-xl tw-font-bold tw-m-0">Add Organization</h2>
				<button type="button" class="tw-text-[#A1A1AA] hover:tw-text-[#FAFAFA] tw-transition-colors" onclick={onClose}>
			...
			<div class="orgs3-add-form">
				...
				<div class="tw-flex tw-justify-end tw-gap-3 tw-mt-6">
					<button type="button" class="tw-text-[#D4D4D8] hover:tw-text-[#FAFAFA] tw-font-bold tw-px-4 tw-py-2" onclick={onClose} disabled={saving}>Cancel</button>
					<button type="button" class="orgs3-submit-btn tw-m-0" onclick={onSubmit} disabled={saving}>
						{#if saving}
