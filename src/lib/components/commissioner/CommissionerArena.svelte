...
<div data-panel="compliance-matrix" class="tenant-matrix-grid federation-matrix-grid tw-w-full tw-h-full tw-p-6 tw-overflow-y-auto tw-bg-[#000000]">
	<div class="bento-grid-container tw-grid tw-grid-cols-1 lg:tw-grid-cols-12 tw-gap-6">
		...
		<section class="st-bento lg:tw-col-span-8 z2-panel siem-panel tw-p-6 tw-flex tw-flex-col tw-min-h-[300px] tw-bg-[#0f172a] tw-border tw-border-[#334155] tw-rounded-none hover:tw-border-nuclear-yellow tw-transition-all" style="background: #0f172a; border: 1px solid #334155;">
			<header class="tw-border-b tw-border-[#334155] tw-pb-3 tw-mb-4 tw-flex tw-items-center tw-justify-between">
				...
				<span class="tw-font-mono tw-text-[10px] tw-text-nuclear-yellow tw-border tw-border-slate-700 tw-bg-slate-900 tw-px-2 tw-py-0.5">
					REALTIME_TELEMETRY
				</span>
			</header>
				...
				<table class="tw-w-full tw-text-left tw-font-mono tw-text-xs tw-text-white tw-border-collapse">
					...
					<tbody>
						{#await engine.loadFederationCompliance()}
							<tr>
								<td colspan="3" class="tw-py-8 tw-text-center tw-text-nuclear-yellow">
							...
							{#each complianceData as item}
								<tr class="tw-border-b tw-border-[#334155] tw-border-opacity-30 tw-bg-[#000000] hover:tw-bg-slate-900 tw-transition-colors">
									...
									<td class="tw-py-2.5 tw-px-2">
										<span class="tw-px-2 tw-py-0.5 tw-rounded-none tw-font-mono tw-font-bold tw-text-[10px] status-dot-indicator {item.complianceStatus === 'green' ? 'tw-bg-[#14b8a6] tw-text-black' : 'tw-bg-nuclear-yellow tw-text-black'}">
		...
		<section class="st-bento lg:tw-col-span-4 z2-panel siem-panel tw-p-6 tw-flex tw-flex-col tw-min-h-[300px] tw-bg-[#0f172a] tw-border tw-border-[#334155] tw-rounded-none hover:tw-border-nuclear-yellow tw-transition-all" style="background: #0f172a; border: 1px solid #334155;">
			<header class="tw-border-b tw-border-[#334155] tw-pb-3 tw-mb-4 tw-flex tw-items-center tw-justify-between">
				<div>
					<h2 class="tw-font-sans tw-text-[#FAFAFA] tw-uppercase tw-tracking-widest tw-text-sm tw-m-0 tw-flex tw-items-center tw-gap-2">
						<Icon name={"game.trophy" as IconName} size={16} class="tw-text-nuclear-yellow" />
							...
							{#each operationsData as item}
								<tr class="tw-border-b tw-border-[#334155] tw-border-opacity-30 tw-bg-[#000000] hover:tw-bg-slate-900 tw-transition-colors">
									...
									<td class="tw-py-2.5 tw-px-2 tw-text-center">
										...
										{:else}
											<span class="tw-px-2 tw-py-0.5 tw-bg-[#1E293B] tw-text-nuclear-yellow tw-font-mono tw-font-bold tw-text-[10px] tw-border tw-border-slate-700 tw-rounded-none">SCHEDULING</span>
										{/if}
									</td>
									<td class="tw-py-2.5 tw-px-2 tw-text-right tw-text-nuclear-yellow tw-font-mono tw-font-bold">{item.teams}</td>
								</tr>
							{/each}
						{/await}
					</tbody>
