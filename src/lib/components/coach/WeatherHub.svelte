...
<div class="tw-h-full tw-w-full tw-min-h-[220px] tw-relative tw-flex tw-flex-col tw-font-mono">
	<!-- Ambient radar rings (subtle watermark) -->
	<svg class="tw-absolute tw-inset-0 tw-w-full tw-h-full tw-pointer-events-none tw-opacity-[0.10]" aria-hidden="true" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid slice">
	...
	{#if vanguardFlags.weatherEnabled}
		<WeatherWidget lat={fieldLat} lng={fieldLng} coordsLabel={weatherCoords} onOpenRadar={() => (isModalOpen = true)} />
	{:else}
		<div
			class="coach-os-well font-mono text-xs p-4 text-center tw-flex tw-h-full tw-flex-col tw-items-center tw-justify-center vanguard-panel"
