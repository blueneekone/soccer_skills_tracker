<script lang="ts">
	...
	$effect(() => {
		if (!browser || !resolvedClubId || !db || !authStore.isAuthenticated) {
	...
	$effect(() => {
		if (!resolvedClubId || !db || !authStore.isAuthenticated) {
			fields = [];
			mapFacilities = [];
			return;
		}
	...
	$effect(() => {
		if (!fieldId || !scheduleDate || !db || !authStore.isAuthenticated) {
			schedules = [];
			return;
		}
		const q = query(
