		...
		<div class="tw-flex tw-shrink-0 tw-items-center tw-gap-2">
			...
			{#if authStore.role !== 'super_admin' && authStore.role !== 'global_admin'}
				<button
					...
					onclick={() => goto('/admin/support-terminal')}
				>
...
<style>
	...
	.vng-app-header__media {
		...
		overflow: hidden;
		background:
			radial-gradient(ellipse at 22% 30%, rgba(20, 184, 166, 0.22) 0%, transparent 55%),
