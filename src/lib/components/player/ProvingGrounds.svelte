...
<script lang="ts">
	...
	const isInputValid = $derived(isBenchmarkInputValid(selectedDrill, inputValue));

	// Whether the current input value earns the performance bonus.
	const bonusEarned = $derived(
		isInputValid && benchmarkBonusEarned(selectedDrill, numInput),
	);

	const previewXP = $derived(
		isInputValid ? computeBenchmarkXp(selectedDrill, numInput) : selectedDrill.baseXP,
	);
	...
	function bonusHint(drill) {
		return formatBenchmarkBonusHint(drill).toUpperCase();
	}
</script>
