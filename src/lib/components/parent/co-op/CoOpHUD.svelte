...
>
	...
	{#if engine.loading}
		...
		>
			<div class="tw-flex tw-flex-col tw-items-center tw-gap-3">
				<div
					class="tw-w-8 tw-h-8 tw-rounded-none tw-border-2 tw-border-[#f59e0b]/30 tw-border-t-amber-500 tw-animate-spin"
				></div>
				<span
					class="tw-font-mono tw-text-[9px] tw-tracking-widest tw-text-[#f59e0b] tw-uppercase tw-animate-pulse"
				>
	...
	>
		...
		>
			...
			<span
				class="tw-font-mono tw-text-[22px] tw-font-bold tw-tracking-tight tw-text-[#f59e0b] tw-leading-none"
			>
				${activeDollars}
			</span>
			<span
		...
		{#if activeBounties.length > 0}
			<div class="tw-flex tw-items-center tw-gap-1.5">
				{#each dotBounties as b (b.id)}
					<div
						class="tw-w-2 tw-h-2 tw-rounded-none tw-bg-[#f59e0b]"
						title={b.title}
					></div>
				{/each}
				{#if extraBounties > 0}
		...
		<button
			...
			class="tw-inline-flex tw-items-center tw-gap-1.5 tw-font-mono tw-text-[10px] tw-font-bold tw-tracking-widest tw-uppercase tw-border tw-border-[#f59e0b] tw-text-black tw-bg-[#f59e0b] tw-rounded-none tw-px-5 tw-py-2.5 tw-transition-all tw-duration-200 hover:tw-bg-[#fbbf24] hover:tw-shadow-[0_0_15px_rgba(251, 191, 36,0.5)] active:tw-scale-95"
		>
