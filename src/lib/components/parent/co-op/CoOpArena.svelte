<script lang="ts">
	...
	function statusChipClass(status: BountyDoc['status']): string {
		switch (status) {
			case 'active':
				return 'tw-bg-[#f59e0b]/15 tw-text-[#f59e0b] tw-border-[#f59e0b]/40';
			case 'verified':
			case 'paid':
...
<div class="tw-bg-[#0F172A] tw-border tw-border-[#1E293B] tw-p-6 tw-flex tw-flex-col tw-gap-6 tw-rounded-[24px]">
	<div class="tw-flex tw-items-center tw-justify-between tw-border-b tw-border-[#1E293B] tw-pb-4">
		...
		<span class="tw-px-2 tw-py-0.5 tw-bg-[#f59e0b]/10 tw-border tw-border-[#f59e0b]/40 tw-text-[#f59e0b] tw-text-[10px] tw-font-mono tw-tracking-widest tw-rounded-none font-bold">
			ESCROW TELEMETRY
		</span>
	</div>
			...
			{:else}
				<div class="tw-flex tw-flex-col tw-gap-3">
					...
					{:else if loadingSources}
						<p class="tw-text-[#f59e0b] tw-font-mono tw-text-xs tw-animate-pulse tw-flex tw-items-center tw-gap-2">
	...
	<section class="tw-flex tw-flex-col">
		<h3 class="tw-text-slate-300 tw-font-bold tw-text-sm tw-mb-3 tw-flex tw-items-center tw-gap-2 tw-font-mono tw-uppercase">
			<Icon name={"game.target" as IconName} size={16} class="tw-text-[#f59e0b]" />
					...
					<div class="tw-bg-[#0B0F19] tw-p-4 tw-border tw-border-[#1E293B] tw-rounded-none">
						...
						{#if bounty.progressTarget && bounty.progressTarget > 0}
							<div class="tw-mb-3">
								<div class="tw-h-1.5 tw-w-full tw-bg-[#1E293B] tw-rounded-none tw-overflow-hidden tw-mb-1">
									<div class="tw-h-full tw-bg-[#f59e0b] tw-transition-all tw-duration-500" style="width: {progress}%"></div>
								</div>
								<div class="tw-flex tw-justify-between tw-text-[10px] tw-font-mono tw-text-slate-400">
									...
									<span class="tw-text-[#f59e0b] tw-font-bold">{progress}%</span>
								</div>
							</div>
						{/if}
						...
						<div class="tw-flex tw-items-center tw-justify-between tw-pt-3 tw-border-t tw-border-[#1E293B]">
							<div class="tw-flex tw-items-end tw-gap-1">
								<span class="tw-text-[#f59e0b] tw-font-bold tw-font-mono tw-text-base">${((bounty.rewardCents ?? 0) / 100).toFixed(2)}</span>
