...
<div class="tw-relative tw-w-full tw-flex tw-flex-col tw-gap-6">
	...
	{#if engine.publicScore}
		...
		>
			...
			<div class="tw-flex tw-items-center tw-justify-between">
				<span class="tw-font-mono tw-text-[10px] tw-tracking-widest tw-text-[#f59e0b] tw-uppercase tw-flex tw-items-center tw-gap-1.5">
					<Icon name={"data.activity" as IconName} size={12} class="tw-text-[#f59e0b]" />
					<span>// MATCH RESULT TELEMETRY</span>
				</span>
				<span
					class="tw-font-mono tw-text-[10px] tw-tracking-widest tw-uppercase tw-px-2.5 tw-py-1 tw-rounded-none tw-border {score.outcome === 'W' ? 'tw-text-[#f59e0b] tw-border-[#f59e0b]/40 tw-bg-[#f59e0b]/10' : score.outcome === 'L' ? 'tw-text-[#ff0055] tw-border-[#ff0055]/40 tw-bg-[#ff0055]/10' : 'tw-text-[#a0a0a0] tw-border-[#a0a0a0]/30 tw-bg-[#a0a0a0]/10'}"
				>
			...
			{#if !engine.attested}
				<div class="tw-flex tw-items-center tw-justify-center tw-gap-2 tw-py-1.5 tw-bg-[#ff0055]/10 tw-border tw-border-[#ff0055]/30">
					<Icon name={"status.warning" as IconName} size={14} class="tw-text-[#f59e0b] tw-animate-pulse" />
					<span class="tw-font-mono tw-text-[9px] tw-tracking-widest tw-text-[#f59e0b] tw-uppercase tw-font-bold">
	...
	{#if engine.attested && engine.lockedMetrics}
		...
		<div
			class="tw-rounded-none tw-border tw-border-[#f59e0b]/40 tw-bg-[#f59e0b]/10 tw-px-5 tw-py-3 tw-flex tw-items-center tw-gap-3"
		>
			<Icon name={"status.check" as IconName} size={16} class="tw-text-[#f59e0b]" />
			<span class="tw-font-mono tw-text-[10px] tw-tracking-widest tw-text-[#f59e0b] tw-uppercase tw-font-bold">
			...
			<div class="tw-rounded-none tw-border tw-border-[#1E293B] tw-bg-[#0B0F19] tw-overflow-hidden">
				...
				<div class="tw-overflow-x-auto">
					<div class="tw-border tw-border-[#334155] tw-bg-[#0f172a] tw-p-4 tw-min-w-0 tw-overflow-x-auto">
						<table class="tw-w-full tw-font-mono tw-text-sm">
						<thead>
						...
						<tbody>
							{#each Object.entries(metrics.playerStats) as [email, stat]}
								<tr class="tw-border-b tw-border-[#1E293B]/50 last:tw-border-b-0 hover:tw-bg-[#14b8a6]/5 tw-transition-colors tw-duration-150">
									...
									<td class="tw-px-3 tw-py-2.5 tw-text-center tw-font-mono tw-text-[11px] tw-text-[#f59e0b] tw-font-bold">{fmt(stat.goals)}</td>
									...
									<td class="tw-px-3 tw-py-2.5 tw-text-center">
										{#if stat.rating !== undefined && stat.rating !== null}
											<span
												class="tw-font-mono tw-text-[11px] tw-font-bold {stat.rating >= 8 ? 'tw-text-[#f59e0b]' : stat.rating >= 6 ? 'tw-text-[#14b8a6]' : 'tw-text-[#ff6b6b]'}"
											>
												{stat.rating}/10
											</span>
										{:else}
