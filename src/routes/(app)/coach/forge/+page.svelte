<script>
	...
	const deployButtonClass = $derived.by(() => {
		...
		if (isDeploying) {
			return `${base} tw-cursor-wait tw-border-cyan-400/45 tw-bg-gradient-to-r tw-from-cyan-700 tw-via-cyan-600 tw-to-teal-700 tw-text-white tw-shadow-[0_0_24px_rgba(34,211,238,0.35)] focus-visible:tw-outline-cyan-300`;
		}
		const idleCyan = `${base} tw-border-cyan-400/50 tw-bg-gradient-to-r tw-from-cyan-600 tw-via-cyan-500 tw-to-teal-500 tw-text-white tw-shadow-[0_0_32px_rgba(34,211,238,0.45)] focus-visible:tw-outline-cyan-300`;
		if (payload.length === 0) {
			return `${idleCyan} tw-cursor-not-allowed tw-opacity-60`;
		}
		return `${idleCyan} hover:tw-border-cyan-300 hover:tw-shadow-[0_0_48px_rgba(34,211,238,0.55)]`;
	});
...
<div class="forge-route tw-mb-10 tw-mt-2 tw-min-h-0 tw-flex-1">
	...
	>
		<header class="tw-mb-10 tw-text-center md:tw-text-left">
			...
			<h1
				class="tw-font-black tw-uppercase tw-tracking-tight tw-text-white tw-drop-shadow-[0_0_24px_rgba(34,211,238,0.25)] md:tw-text-5xl tw-text-3xl"
			>
		...
		<section class="tw-mb-12" aria-labelledby="forge-armory-heading">
			...
			>
				{#each ARMORY_DRILLS as drill (drill.id)}
					<article
						class="tw-group tw-relative tw-flex tw-flex-col tw-overflow-hidden tw-rounded-xl tw-border tw-border-white/[0.07] tw-bg-slate-950/45 tw-p-4 tw-backdrop-blur-sm tw-transition hover:tw-border-cyan-500/25 hover:tw-shadow-[0_0_28px_rgba(6,182,212,0.12)]"
					>
		...
		<section aria-labelledby="forge-payload-heading">
			...
			>
				<div class="tw-mb-8 tw-text-center">
					...
					<p
						class="forge-bounty-total tw-mt-6 tw-font-black tw-tabular-nums tw-text-cyan-400 tw-text-4xl tw-tracking-tight tw-shadow-[0_0_15px_rgba(6,182,212,0.5)] md:tw-text-5xl"
					>
