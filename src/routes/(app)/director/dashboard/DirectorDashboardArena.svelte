	...
	<div class="tw-max-w-[1920px] tw-mx-auto tw-flex tw-flex-col tw-gap-8 tw-min-h-full">
		...
		{#if engine.activeTab === 'home'}
			<section class="tw-flex tw-flex-col tw-gap-6 tw-w-full">
				...
				<div class="director-hud-grid tw-grid tw-w-full tw-gap-4 tw-mt-2" style="grid-template-columns: repeat(auto-fit, minmax(min(100%, clamp(280px, 30vw, 350px)), 1fr));">
		...
		{:else if engine.activeTab === 'comms'}
			<section class="director-bento-grid-container tw-grid tw-w-full tw-gap-8" style="grid-template-columns: repeat(auto-fit, minmax(min(100%, clamp(280px, 30vw, 350px)), 1fr));">
				<section
