<script lang="ts">
	import { SystemSettingsEngine } from './SystemSettingsEngine.svelte.js';
	import SystemSettingsArena from './SystemSettingsArena.svelte';
	
	const engine = new SystemSettingsEngine();
	engine.subscribe();
</script>

<div
	class="pd-page-root tw-flex tw-flex-col tw-w-full tw-min-w-0 tw-flex-1 tw-min-h-0 tw-gap-[clamp(24px,3vw,36px)] tw-bg-[#0B0F19] tw-text-[#FAFAFA] dark-form-surface cc-root tw-box-border tw-overflow-y-auto admin-os-root"
