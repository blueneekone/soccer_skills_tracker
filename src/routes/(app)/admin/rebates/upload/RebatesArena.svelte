...
{#if engine.phase === 'idle' || engine.phase === 'parsing'}
	...
	>
		<div class="drop-icon tw-flex tw-justify-center tw-mb-2">
			<Icon name={"comm.file-text" as IconName} size={36} class="tw-text-[#f59e0b]" />
		</div>
		<p class="drop-label">Drop a CSV file here, or</p>
		<label class="btn-browse tw-inline-flex tw-items-center tw-gap-2 tw-bg-[#f59e0b] tw-text-void-black tw-font-bold tw-px-4 tw-py-2 tw-rounded-none tw-cursor-pointer">
...
{:else if engine.phase === 'previewing'}
	<div class="preview-section bento-grid-container tw-grid tw-grid-cols-1 lg:tw-grid-cols-12 tw-gap-6">
		...
		<div class="action-row tw-flex tw-items-center tw-gap-4 tw-mt-4">
			...
			<button
				class="btn-submit tw-bg-[#f59e0b] tw-text-void-black tw-border-[#f59e0b] tw-font-extrabold tw-px-6 tw-py-2.5 tw-rounded-none tw-flex tw-items-center tw-gap-2"
