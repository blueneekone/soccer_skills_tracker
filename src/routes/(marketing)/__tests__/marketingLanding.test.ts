...
describe('CDO Protocol: Marketing Landing Page Audit', () => {
	it('MUST enforce the strict 12-column asymmetric Bento Grid (6/4/2)', () => {
		// The `features` array should define the asymmetrical col-spans.
		expect(PageSource).toContain("cols: 'md:tw-col-span-6'");
		expect(PageSource).toContain("cols: 'md:tw-col-span-4'");
		expect(PageSource).toContain("cols: 'md:tw-col-span-2'");
		
		// It should NOT contain three symmetrical 4-columns
		const count4Col = (PageSource.match(/md:tw-col-span-4/g) || []).length;
		expect(count4Col).toBe(1);
	});
