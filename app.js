// --- VERSION CHECK ---
console.log("--- APP LOADED: FIX V2 (Popup Auth) ---");
...
// Attempt persistence (Offline support)
...
if(loginGoogleBtn) {
    loginGoogleBtn.addEventListener("click", () => {
        console.log("Google Login Clicked - Starting Popup...");
        const provider = new GoogleAuthProvider();
        signInWithPopup(auth, provider) // KEY FIX: Popup instead of Redirect
            .then((result) => {
                console.log("Google Sign In Success:", result.user.email);
            })
            .catch((error) => {
                console.error("Google Error:", error);
                authErrorMsg.textContent = "Google Error: " + error.message;
...
if (saveRosterBtn) {
    saveRosterBtn.addEventListener("click", async () => {
        ...
        if (!targetTeamId || targetTeamId === "") { targetTeamId = currentCoachTeamId; }
        if (!targetTeamId || targetTeamId === "unassigned" || targetTeamId === "all") { return alert("Error: Could not determine which team to save to. Please ensure a team is selected."); }
