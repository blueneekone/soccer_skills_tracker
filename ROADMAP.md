...
- ~~War Room HQ hero + sidebar nav (`/coach/tactical`)~~ **Done** — single surface **WARROOM-SINGLE-SURFACE**; PS-C05 removed, redirect `/coach/tactics-board` → `/coach/tactical`
...
| **WARROOM-SINGLE-SURFACE** | **Done** | One coach tactical surface — `/coach/tactical` canonical (`TacticalArena` + `createTacticalWarRoom`); `/coach/tactics-board` redirect; `TacticalCommandBoard` retired; PS-C05 removed; PS-C04 `nav_visible=true`; sidebar War Room + HQ hero | `warRoomSingleSurface.test.ts` · `personaFunctionalMvp.test.ts` · `productSurfaceRegistry.test.ts` |
...
| **COACH-DASHBOARD-LIVE-DATA** | **Done** | Daily Intel `/coach` — remove `READINESS_ROSTER` mock operatives; readiness matrix + squad uptime derive from live roster, `player_stats`, and VPC queue | `coachDashboardLiveData.test.ts` · **hosting** |
| **COACH-ROSTER-IMPORT** | **Done** | Coach bulk CSV import on Team Ops Roster tab (`CoachRosterImportPanel` + `secureBulkAddPlayers`); Daily Intel discoverability link; director `ingestRoster` unchanged | `rosterCsvParse.test.ts` · `coachRosterImport.test.ts` · `adminOpsBulkRoster.test.js` · deploy **`npm run deploy:platform`** |
| **DIRECTOR-ROSTER-INGEST-UI** | **Done** | Admin org team roster — `RosterIngestPanel` calls director-only `ingestRoster` (CSV/JSON/PDF + invite codes); coaches stay on Team Ops `secureBulkAddPlayers` | `rosterIngestPanel.test.ts` · deploy **`npm run deploy:integrations`** |
...
| 4.15b | **Done** | Phase 3 emergency break-glass — `emergency` channel type; `emergencyClubBroadcast` callable + `priority: 'emergency'` on `team_broadcasts`; high-priority FCM + 4.16a SMS hook; `CommsEmergencyChannel` with confirm dialog | `commsPhase3b.test.ts` · `functions/__tests__/commsPhase3b.test.js` |
...
| COMMS-STANDARDS-DOC | **Done** | Comms platform standards (drift prevention) — `COMMS_PLATFORM_STANDARDS.md`, `COMMS_UX_NAV_SPEC.md`, `COMMS_CALENDAR_INTEGRATION.md`; canon/hub/matrix/registry sync; `commsStandardsDoc.test.ts` | `commsStandardsDoc.test.ts` |
| COMMS-PARENT-CIRCLE-POLICY | **Done** | Parent Circle = parent-peer only — `provisionParentLoungeChannel` parents-only `memberIds` + `channelType: parent_lounge`; `sendChannelMessage` blocks non-parent; `ParentLoungePanel` copy + `channelTypes` parent-only post | `commsParentCircle.test.ts` |
| COMMS-PARENT-COACH-DM | **Done** | Bilateral parent↔coach DM — `parentCoachDmOps.js` (`sendParentCoachMessage`, `listParentCoachDmThreads`); `clubs/{clubId}/parent_coach_threads/{threadId}/messages`; `includeAdOnParentDms` club flag + disclosure banner; `ParentCoachDmPanel` + hub Families rail | `commsParentCoachDm.test.ts` · `functions/__tests__/commsParentCoachDm.test.js` |
| COMMS-COST-HOTFIX | **Done** | Firestore read amplification hotfix — parent announcements single-query + debounced acks; dashboard strip cache; sponsor channel deferred listener; lounge/staff/household limit 50 | `commsCostHotfix.test.ts` |
...
| COMMS-UX-RECOVERY | **Shipped — owner VA pending** | Team Ops native compose (no link farm); hub removes space `<select>` + category iconChar; `CommsThreadShell` + `CommsLogisticsThread`; messages strap copy; Part A (**COACH-NAV-DEMOTE**) shipped separately; **superseded for coach team workflows by COMMS-TEAMOPS-UNIFY** (single `/messages` CTA allowance removed) | `commsUxPremium.test.ts` · `commsNav20.test.ts` · `commsPhase2.test.ts` · owner screenshots @390px + desktop |
| COMMS-TEAMOPS-UNIFY | **Shipped — owner VA pending** | Coach team comms canonical on `/coach/logistics` Comms tab — `CoachTeamCommsPanel` (`ec-page` + `coach-os-panel` flat analytics) embeds compose + logistics sub-tabs + threads; coach `/messages` redirects to Team Ops; field pin → `/coach/logistics?tab=comms`; parent `/messages` unchanged; no comms-hub z4 strap on Team Ops embed | `commsUxPremium.test.ts` · `commsPhase2.test.ts` · `commsPhase1.test.ts` · `commsSprint47.test.ts` · `commsNav20.test.ts` · **owner screenshots @390px + desktop required before Done** |
| COMMS-PREMIUM-V2 | **Shipped — owner VA pending** | Kill pill tabs on coach comms — `CommsWorkspaceShell` (coach-team rail + mobile sheet); inbox-first default `parent_coach_dm`; `ParentCoachDmPanel` list-detail split; underline Team Ops + logistics embed subs; forbid `border-radius: 999px` on coach comms surfaces | `commsUxPremium.test.ts` · `commsParentCoachDm.test.ts` · `commsNav20.test.ts` · **owner screenshots @390px + desktop required before Done** |
| COMMS-DOCS-SYNC | **Done** | Vision/QA/nav docs aligned to COMMS-TEAMOPS-UNIFY + COMMS-PREMIUM-V2 — coach Team Ops Comms embed canonical; `OWNER_QA_CHECKLIST` QA-146 rewrite; `PLATFORM_NAVIGATION_CANON` Team Comms pin; `FUNCTIONAL_MVP` + acquisition docs; regression tests updated | `commsSprint41.test.ts` · `commsStandardsDoc.test.ts` · `commsQaChecklist.test.ts` · `commsUxPremium.test.ts` |

**Epic 4 runs parallel to Epic 3** after **3.2** ships. **Epic 4.1–4.16d + COMMS-CLOSE + COMMS-STANDARDS-DOC + COMMS-PARENT-CIRCLE-POLICY + COMMS-PARENT-COACH-DM + COMMS-COST-HOTFIX + COMMS-NAV-2.0 + COMMS-SPONSOR-REHOME + COMMS-VOICE-V1 + COMMS-PRE-QA-WIRE + COMMS-DOCS-SYNC Done.** Comms functions deployed to `sports-skill-tracker-dev` via `npm run deploy:comms`.
