...
function computeReward(logDoc, inferLog, playerStats, hasGritAward) {
  // Engagement: tanh(earnedXP / expectedXP).
  // expectedXP derived from duration × intensity multiplier (same as calculateTrainingSessionEarnedXp logic).
  const earnedXP = Number(logDoc.earnedXP ?? 0);
  const intensityMultiplier = logDoc.intensity === 'high' ? 1.5 : logDoc.intensity === 'low' ? 0.8 : 1.0;
  const expectedXP = Math.max(1, Number(logDoc.duration ?? 30) * intensityMultiplier * 2);
