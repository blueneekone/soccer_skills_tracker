 ...
 */
exports.sendParentCoachMessage = onCall({region: REGION, cors: [/soccerskillstracker\\.com$/, /localhost:[0-9]+$/]}, async (request) => {
  if (!request.auth) {
 ...
 */
exports.listParentCoachDmThreads = onCall({region: REGION, cors: [/soccerskillstracker\\.com$/, /localhost:[0-9]+$/]}, async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'Sign in required.');
  }
