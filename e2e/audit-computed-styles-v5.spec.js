...
const EMULATOR_FIRESTORE_URL = 'http://localhost:8080/v1/projects/sports-skill-tracker-dev/databases/(default)/documents/users/mock-coach-uid';
...
test.describe('SSTracker Elite Visual Compliance Suite', () => {
    ...
    test('Assert Strict "Nuclear Americana Tech Noir" Compliance', async ({ page }) => {
        ...
        await page.addInitScript(() => {
            const firebaseApiKey = "AIzaSyCiBoemXJHTkTnujTwM1vOJc4FrVZF8Lw8";
        ...
        const canvasBg = await page.evaluate(() => {
            ...
            return { color: window.getComputedStyle(el).backgroundColor, className: el.className };
        });
        // Strict check: Must match rgb(0, 0, 0) - absolute Void Black
        expect(canvasBg).toEqual({ color: 'rgb(0, 0, 0)', className: expect.any(String) });
