// SSTracker Master Visual Compliance Audit Engine (v6)
...
async function seedMockProfile(targetPersona, targetUid) {
    const EMULATOR_FIRESTORE_URL = `http://localhost:8080/v1/projects/sstracker-nexus/databases/(default)/documents/users/${targetUid}`;
    const payload = {
        fields: {
            uid: { stringValue: targetUid },
            role: { stringValue: targetPersona },
    ...
    try {
        ...
        if (res.ok) {
            console.log(`[+] Firestore Emulator successfully seeded with completed profile for ${targetPersona} (${targetUid}).`);
        } else {
            console.error(`[-] Failed to seed Firestore Emulator for ${targetPersona}:`, res.statusText);
        }
    } catch (err) {
        console.error(`[-] Network error seeding Firestore Emulator for ${targetPersona}:`, err.message);
    }
}

test.describe('SSTracker Elite Visual Compliance Suite', () => {
    const targetPersona = (process.env.AUDIT_TARGET || 'coach').toLowerCase();
    const targetRoute = `/${targetPersona}/dashboard`;
    const targetUid = `${targetPersona}-telemetry-uid`;

    test.beforeAll(async () => {
        await seedMockProfile(targetPersona, targetUid);
    });

    test('Assert Strict "Nuclear Americana Tech Noir" Compliance', async ({ page }) => {
        // Enforce the Zero-Touch CSO Protocol: Inject authenticated state directly before page loads
        ...
        }, { uid: targetUid, role: targetPersona });

        console.log(`[Audit] Navigating to ${targetRoute} for validation...`);
        await page.goto(targetRoute);
        
        // Wait explicitly for page hydration rather than generic networkidle (prevents Firestore snapshot timeouts)
        ...
        for (const vp of viewports) {
            ...
            await page.waitForTimeout(250); // Wait for CSS transition kinetics (150-250ms)
